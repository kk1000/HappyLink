...
public class MainActivity extends Activity {
	...
	private void init()
	{
		...
		// 获取开始按钮
		startButton = (Button) findViewById(R.id.main_begin);
		// 获取返回按钮
		returnButton = (Button) findViewById(R.id.main_return);
		// 初始化音效
	...
	private void saveTime(){
		//从文件中获取count标记，以动态判断条件
		int count = preferences.getInt("count", 0);
		if(count < 5){
			editor.putInt("gametime"+count, gameTime+1);
			...
			for (int i = 0; i < 5; i++) {
				scores[i] = preferences.getInt("gametime"+i, 0);				
			}
...
//				editor.putInt("gametime"+i, scores[i+1]);
//				editor.commit();
//			}
			
			//存储后五位
			for(int n=5;n>0;n--){
				editor.putInt("gametime"+(n-1), scores[n]);
				editor.commit();
			}
		}
	}
